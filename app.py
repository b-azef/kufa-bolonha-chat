...
st.set_page_config(
    page_title="مساعد جامعة الكوفة", page_icon="📊", layout="centered"
)
...
# 3. إعدادات روابط Google Sheets وجلب البيانات
...
def load_all_academic_data():
  ...
  for mat in materials:
    try:
      ...
      final_df = pd.merge(
          final_df, df_mat, on="username", how="left", suffixes=("", f"_{mat}")
      )
    except Exception:
      continue
...
if not st.session_state.logged_in:
  st.markdown(
      "<h2 style='text-align: center; color: white;'>🏛️ بوابة مسار بولونيا -"
      " جامعة الكوفة</h2>",
      unsafe_allow_html=True,
  )
  st.markdown(
      "<p style='text-align: center; color: #888;'>Log in</p>",
      unsafe_allow_html=True,
  )
  ...
  if submit_button or (input_user and input_pass):
    match = df_students[
        (df_students["username"] == input_user)
        & (df_students["password"] == input_pass)
    ]
    if not match.empty:
...
with col_name:
  st.markdown(
      f"<h3 style='color: white; margin:0;'>مرحباً بك:"
      f" {current_student.get('student_name', 'طالب جامعة الكوفة')} 👋</h3>",
      unsafe_allow_html=True,
  )
with col_logout:
...
st.markdown(
    "<h4 style='color: #a855f7;'>📊 موقف غيابات المواد الحالي:</h4>",
    unsafe_allow_html=True,
)
  ...
  for col_idx in range(3):
    ...
    with cols[col_idx]:
      ...
      st.markdown(
          ...
          f" color: #aaa;'>{status}</p>",
          unsafe_allow_html=True,
      )

st.markdown("---")

# 🧠 التوجيهات والمهام الوظيفية لـ Gemini
...
1. التفاعل الفردي: عندما يسألك الطالب عن نفسه، درجاته، غياباته، تبليغاته أو مستواه الأكاديمي، أجب بسلاسة، ود، وطبيعية كاملة.
...
3. التعامل مع الغيابات والتبليغات:
   - أعمدة الـ `attendance` تمثل "عدد أيام الغياب الفعلي" للمادة المعنية، واذكر الرقم صافياً دون شرح مطول لنظام بولونيا.
   - الغيابات >= 3 إنذار أول 🟡، >= 5 تحذير ثانٍ 🟠، >= 7 تحذير نهائي وفصل 🔴.
   - إذا سألك الطالب عن التبليغات أو الامتحانات أو أي إعلانات جديدة، اقرأ التبليغات المرفقة وأخبره بها بشكل واضح ومباشر.

4. خط أحمر صارم للمهام الخارجة عن وظيفتك: إذا طلب منك الطالب حل مسألة رياضية، كتابة تقرير، حل واجب، شرح درس، أو أي عمل دراسي نيابة عنه؛ اعتذر منه بأدب ووجهه لأدوات الذكاء الاصطناعي العامة.
5. إياك أن تذكر لقب "أبو لينا" في الشات نهائياً، واكتفِ بذكر الاسم المطور "علي حكمت حسن".
"""

# 5. الترحيب الأولي المنظم
if "chat_history" not in st.session_state or st.session_state.chat_history is None:
  student_name = current_student.get("student_name", "طالبنا العزيز")
  welcome_msg = (
      f"أهلاً بك يا {student_name} في مساعد جامعة الكوفة الأكاديمي للمواد الست."
      " أنا هنا لمساعدتك في تتبع درجاتك، غياباتك، والإعلانات والتبليغات"
      " الرسمية. كيف يمكنني مساعدتك اليوم؟"
  )
...
if isinstance(st.session_state.chat_history, list):
  ...
  for msg in st.session_state.chat_history:
    ...
    chat_html += (
        f'<div class="msg-wrapper {role_class}"><div'
        f' class="bubble">{msg["content"]}</div></div>'
    )
  chat_html += "</div>"
  st.markdown(chat_html, unsafe_allow_html=True)

# 7. استقبال أسئلة الطالب وتمرير التبليغات لنموذج Gemini
if user_query := st.chat_input("Ask me..."):
  ...
  # تمرير التبليغات للذكاء الاصطناعي
  announcements_data = (
      announcements_df.to_dict(orient="records")
      if not announcements_df.empty
      else "لا توجد تبليغات حالية."
  )

  chat_prompt = f"""{STRICT_ACADEMIC_RULES}
    بيانات الطالب الحالي: {str(current_student)}
    قائمة التبليغات الرسمية الصادرة: {str(announcements_data)}

    سجل المحادثة الكامل حتى الآن:
    {conversation_context}

    تذكر: أجب بسلاسة واذكر التبليغات بدقة إذا سأل عنها الطالب، مع الالتزام التام بالقواعد."""
