...
# 🧠 القواعد الصارمة والمصححة لغوياً لقطع خلط الغياب بالحضور نهائياً
...
3. تصحيح حاسم لمفهوم الغياب (مسار بولونيا):
   - أعمدة الـ `attendance` (مثل `math_attendance` و `prog_attendance`) تمثل **"عدد أيام الغياب الفعلي للطالب"** وليست درجة حضور!
   - في نظام بولونيا **لا توجد درجة حضور تضاف للسعي إطلاقاً**. الرقم صفر (0) في الغيابات يعني أن الطالب ممتاز ولم يغب أي يوم، ولا يعني أن درجته صفر!
   - تعامل مع هذه الأرقام كعقوبات وإنذارات فقط: (عدد الغيابات >= 3 إنذار أول 🟡، >= 5 تحذير ثانٍ 🟠، >= 7 تحذير نهائي وفصل 🔴). إياك أن تنصح الطالب برفع "درجة الحضور" أو تخلط بين المفهومين.
...
if user_query := st.chat_input("Ask me..."):
    ...
    تذكر القواعد حركياً: رقم 0 في الغيابات يعني عدم وجود غيابات وهو ممتاز، ولا توجد درجات حضور في مسار بولونيا."""
